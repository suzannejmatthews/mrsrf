...
if ($#ARGV != 1) {
  print "usage: perl copy_hash.pl <num proc> <number of trees>\n";
...
$num_proc = $ARGV[0];
$num_trees = $ARGV[1];
my @all_bp_files = `cd $FILEDIR; ls`;
my $size = scalar(@all_bp_files);
my $outname = "hashtable";
print "size is: $size\n";

print "found: $size comparable bipartitions\n";
`cat $FILEDIR/* > $outname`;
print "generating $num_proc files necessary for second map reduce process\n";
...
if ($remainder == 0 ) {
	...
	print WRITE2 "$outname $size $id $num_trees_pfile\n";
	$id += $num_trees_pfile;
    }
    close(WRITE2);
	...
	print WRITE2 "$outname $size $id $newlimit\n";
	$id += $newlimit;
    }
